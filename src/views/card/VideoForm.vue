<template>
  <div class="form-div">
    <div class="form-container">
      ...
      <div class="choose-time">
        <span
          >个人视频<span style="font-size:12px;color:#8e8f90"
            >(视频格式限制为MP4,大小不超过40MB)</span
          ></span
        >
        <van-switch
          active-color="#294a7b"
          ...
          size="small"
          v-model="isPoeSwitch"
          @change="choosePoeSwitch"
        ></van-switch>
      </div>
      <div
        style="width:90%;margin:20px"
        v-if="
          playerOptions.sources && playerOptions.sources[0].src && isPoeSwitch
        "
      >
        <video-player
      ...
      <div class="video-div" v-if="isPoeSwitch">
        <van-uploader
          class="upload-div"
        ...
        <van-progress
          ...
          v-if="loading == true"
        />
        <div style="text-align:center" v-if="isPoeSwitch">
          <van-button
            type="default"
            size="small"
            class="video-block"
          ...
          <van-button
            ...
            class="video-block"
            @click="beforeVideo"
            v-if="videoList.length != 0 && loading == false && showBtnSumbit"
            :disabled="loading"
      ...
      >
        <span
          >企业视频<span style="font-size:12px;color:#8e8f90"
            >(开启后，企业视频将显示为我的视频)</span
          ></span
        >
        ...
        <van-switch
          ...
          size="small"
          v-model="isEntSwitch"
          @change="chooseEntSwitch"
        ></van-switch>
      </div>
      <div
        style="width:90%;margin:20px"
        v-if="
          playerOptionsTwo.sources &&
            playerOptionsTwo.sources[0].src &&
            isEntSwitch
        "
      >
        <video-player
...
export default {
  data() {
    return {
      ...
      //换版之后
      isPoeSwitch: false, //个人视频开关
      isEntSwitch: false, // 企业视频开关
      deteleBtn: false, //点击删除个人视频之后
    };
  },
  methods: {
    //点击切换个人视频开关
    choosePoeSwitch(val) {
      if (val == true) {
        this.isEntSwitch = false;
        this.isNow = "1";
      } else {
        this.isEntSwitch = true;
      }
    },
    ...
    chooseEntSwitch(val) {
      if (val == true) {
        this.isPoeSwitch = false;
        this.isNow = "2";
      } else {
        this.isPoeSwitch = true;
      }
    },
    //弹窗确认
    chooseCon() {
    ...
    sumbitBtn() {
      ...
      if (this.isNow == 1 && this.isPoeSwitch) {
        console.log("如果是个人信息", this.isNow);
        if (this.model.videoUrl == "") {
          console.log("如果链接为空，跑这里", this.model);
          Notify({
            type: "warning",
            message: "请上传视频",
          });
          ...
          console.log("如果不为空，跑这里", this.model);
          this.updateBussiness(this.model);
        }
      } else if (this.isNow == 2) {
        this.model.videoUrl = "";
        this.updateBussiness(this.model);
      } else if (this.isNow == 1 && !this.isPoeSwitch) {
        this.model.videoUrl = "";
        this.updateBussiness(this.model);
      }
    ...
    getComPage() {
      this.$http.getComPage().then((resData) => {
        ...
        if (this.deteleBtn) {
          ...
          this.isEntSwitch = true;
          this.isPoeSwitch = false;
          this.deteleBtn = false;
          return;
        }
        if (this.model.videoUrl == "" && data.videoUrl == "") {
          this.isNow = "1";
          this.isPoeSwitch = true;
        }
      });
